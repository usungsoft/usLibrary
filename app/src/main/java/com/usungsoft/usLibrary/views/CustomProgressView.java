...
public class CustomProgressView extends View {
    ...
    private final int[] mDefaultColorList = new int[]{Color.RED, Color.YELLOW, Color.GREEN};
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mShape == Shape.ARC)
            ...
            setMeasuredDimension(width, (int) (0 + highStroke / 1.5)); //setMeasuredDimension(width, (int) Math.max(highStroke, height));
        }
        updateShader();
    }
    ...
    public void applyGradient(int[] colorList) {
        ...
        if (mGradientColorList != null && mProgressDirection == Direction.FROM_RIGHT)
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        int direction = typedArray.getInt(R.styleable.CustomProgressView_pvDirection, 0);
        mProgressDirection = direction == 0 ? Direction.FROM_LEFT : Direction.FROM_RIGHT;

        int shape = typedArray.getInt(R.styleable.CustomProgressView_pvShape, 0);
        mShape = shape == 0 ? Shape.ARC : shape == 1 ? Shape.CIRCLE : Shape.LINE;

        mProgress = typedArray.getFloat(R.styleable.CustomProgressView_pvProgress, mProgress);
        mBackgroundWidth = typedArray.getDimension(R.styleable.CustomProgressView_pvBackgroundWidth, dpToPx(context, 28));
        mProgressWidth = typedArray.getDimension(R.styleable.CustomProgressView_pvProgressWidth, dpToPx(context, 28));
        mBackgroundColor = typedArray.getColor(R.styleable.CustomProgressView_pvBackgroundColor, Color.parseColor("#CCCCCC"));
        mProgressColor = typedArray.getColor(R.styleable.CustomProgressView_pvProgressColor, Color.RED);
        mAnimationDuration = typedArray.getInt(R.styleable.CustomProgressView_pvAnimateDuration, mAnimationDuration);
        ...
        applyGradient(mDefaultColorList);
    }
